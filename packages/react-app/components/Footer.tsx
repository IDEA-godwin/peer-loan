...
const navigation = [
  {
    name: 'home',
    icon: (props: FooterNavigationProps) => (
  ...
  },
  {
    ...
    icon: (props: FooterNavigationProps) => (
      <svg xmlns="http://www.w3.org/2000/svg" width="2.5em" height="2.5em" viewBox="0 0 24 24" {...props}>
        <path d="M14.807 5.593c.48.043.977.128 1.478.266V7.22c-.257-.188-.891-.57-2.07-.57s-1.384.506-1.341.763c-.034.116.086.399.814.6c.159.043.352.078.574.12c1.119.202 2.88.527 2.88 2.229c0 1.178-.99 1.89-2.335 2.036l.15.887h-1.329l.142-.883a6.5 6.5 0 0 1-2.199-.617v-1.423c.609.386 4.05 1.598 4.23 0c.056-.523-.664-.677-1.526-.866c-1.213-.257-2.704-.583-2.704-2.078c0-1.123.926-1.702 2.194-1.822l-.137-.883h1.333l-.15.879zm-3.236 11.55a4.29 4.29 0 0 0 3.93-2.572h1.371a5.571 5.571 0 1 1-7.015-7.015v1.371a4.286 4.286 0 0 0 1.714 8.216" />
      </svg>
    ),
  },
  {
...
export default function Footer({currentPage, switchPage}: Props) {
  return (
    <footer className="bg-gypsum mt-auto border-black absolute bottom-0 w-full">
      <div className="mx-auto max-w-7xl py-2 px-8">
        <div className="flex justify-between items-center footer-items">
          {navigation.map((item) => (
            <FooterOption key={item.name} optionName={item.name} icon={item.icon} activePage={currentPage} click={switchPage}  />
          ))}
        </div>
      </div>
    </footer>
